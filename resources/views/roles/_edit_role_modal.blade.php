    ...
    <div class="modal-dialog modal-xl modal-simple modal-dialog-centered modal-add-new-role">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <!-- Add role form -->
                <form id="editRoleForm" class="row g-3"  action="{{route($route .'.update', $role->id)}}" method="post">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="col-12">
                        ...
                        <div class="table-responsive">
                            <table class="table table-flush-spacing">
                                ...
                                @foreach($role_permissions[$role->id] as $row)
                                    <tr>
                                                ...
                                                @foreach($row['permissions'] as $permissionID => $permission)
                                                    <div class="form-check mb-0 me-4 me-lg-12">
                                                        <input class="form-check-input" type="checkbox" name="permission[]"
                                                               value="{{ $permissionID }}" id="check{{ $permissionID }}"
                                                               @if($permission['checked']) checked @endif />
                                                        <label class="form-check-label" for="check{{ $permissionID }}">
